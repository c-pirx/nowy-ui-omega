...
Świeże `rules`, `ajax_url`, `nonce` i `i18n` są przekazywane do nowego modułu przez WordPress. Stary kontroler `omega-kalk-js` zostaje zdjęty z kolejki na nowym szablonie, aby nie dodawał drugiego handlera wysyłki. Oryginalny endpoint `omega_kalk_lead`, wymagane pola payloadu, polityka prywatności oraz walidacja i wysyłka pozostają odpowiedzialnością istniejącej wtyczki.

Obliczenie jest anonimowe i nie wywołuje endpointu. Dopiero wybór „Wyślij wynik do potwierdzenia”, uzupełnienie kontaktu i zatwierdzenie formularza wysyła zapamiętane parametry wyceny, kwotę oraz `wiadomosc`. Wiadomość jest opcjonalna, ograniczona do 3000 znaków. Zmiana parametrów unieważnia poprzedni wynik. Podgląd na localhost i statycznym GitHub Pages nie wysyła wiadomości.

Adapter ogłasza `window.OmegaMGConfirmation.enabled` i dla oznaczonych żądań `omega_redesign_confirmation=1` do oryginalnej akcji instaluje filtr `wp_mail`. Filtr dopisuje oczyszczoną wiadomość i podsumowanie do treści tekstowej lub HTML. Nie zastępuje handlera, nie zmienia odbiorcy, nagłówków ani odpowiedzi serwera. Oryginalny handler nadal musi zweryfikować nonce, zgodę i kontakt przed wywołaniem poczty. Podsumowanie jest opisane jako orientacyjne wyliczenie klienta wymagające potwierdzenia, nie jako zatwierdzona cena. Integracja wymaga, aby oryginalna wtyczka wysyłała przez standardowe `wp_mail`; zewnętrzny mechanizm omijający ten filtr wymaga dopasowania adaptera.
...
**Wymagana weryfikacja na kopii WordPressa:** potwierdź aktualną obecność `window.OmegaKalkData`, naliczanie zgodne z oryginalnymi regułami, brak wysyłki przy samym obliczeniu oraz jedną wysyłkę do istniejącego endpointu dopiero po zatwierdzeniu kontaktu. W otrzymanym e-mailu sprawdź wiadomość i komplet parametrów odpowiadających pokazanemu wynikowi. Sprawdź też odpowiedzi błędów, wygasły nonce, działanie poczty/odbiorcy i konfigurację cache. Cache całej strony nie może serwować nieaktualnego nonce dłużej niż dopuszcza pierwotna wtyczka.
